...
namespace BingImageDownloadServiceForWindows
{
	public partial class BingImageDownloadServiceControl : ServiceBase
	{
		...
		private async void BackgroundWorker()
		{
			...
			string strWindowsSpotlightDownloadService = " Windows聚焦API图片下载服务 ";
			...
			while (!_cancellationTokenSource.IsCancellationRequested)
			{
				...
				if (!_isPaused)
				{
					try
					{
						...
						if (ClassLibrary.ShareClass._windowsSpotlightCopyService)
						{
						...
						{
							await BingImageDownloadServiceForWindows.WindowsSpotlightDownloadService.WindowsSpotlightDownloadServiceAsync(_cancellationTokenSource.Token);
							if (!strTemp.Contains(strWindowsSpotlightDownloadService))
							{
								strTemp += strWindowsSpotlightDownloadService;
							}
						}
						else
						{
							if (_logCount % intFrequency == 0)
							{
								ClassLibrary.MyLogHelper.LogSplit($"根据配置，{strWindowsSpotlightDownloadService}不开启");
							}
							strTemp = strTemp.Replace(strWindowsSpotlightDownloadService, "");
						}
