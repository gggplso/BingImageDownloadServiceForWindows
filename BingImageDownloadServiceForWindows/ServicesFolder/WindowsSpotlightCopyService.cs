...
{
    internal class WindowsSpotlightCopyService
    {
        /// <summary>
        /// 创建一个信号量，用于同步并发访问
        /// </summary>
        ...
        /// <summary>
        /// 异步方法，现在接受一个取消令牌（Windows聚焦图片复制服务）
        /// </summary>
        /// <param name="cancellationToken"></param>
        /// <returns></returns>
        internal static async Task WindowsSpotlightCopyServiceAsync(CancellationToken cancellationToken)
        {
            // 定义配置文件路径和相关日志信息
            var logTitle = "Windows聚焦图片复制(服务)";
                ...
                if (bingImageDownloadService.BingImageSetting != null)
                {
                    try
                    {
                        if (bingImageDownloadService.WindowsSpotlightCopy())
                        {
                            ...
                            ClassLibrary.MyLogHelper.LogSplit(ClassLibrary.ShareClass._logPath, logTitle, "Windows聚焦图片复制完成。", logType, logCycle);
                        }
                        else
                        {
                            ClassLibrary.MyLogHelper.LogSplit(ClassLibrary.ShareClass._logPath, logTitle, "Windows聚焦图片复制失败。", "Error_" + logType, logCycle);
                        }
                    }
                    catch (Exception ex)
                    {
            ...
            {
                ClassLibrary.MyLogHelper.LogSplit(ClassLibrary.ShareClass._logPath, logTitle, " Windows聚焦图片复制 服务已取消", logType, logCycle);
