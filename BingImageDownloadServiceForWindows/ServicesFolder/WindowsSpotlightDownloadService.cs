...
{
    internal class WindowsSpotlightDownloadService
    {
        /// <summary>
        /// 创建一个信号量，用于同步并发访问
        /// </summary>
        ...
        /// <summary>
        /// 异步方法，现在接受一个取消令牌（Windows聚焦API图片下载服务）
        /// </summary>
        /// <param name="cancellationToken"></param>
        /// <returns></returns>
        internal static async Task WindowsSpotlightDownloadServiceAsync(CancellationToken cancellationToken)
        {
            // 定义配置文件路径和相关日志信息
            var logTitle = "Windows聚焦API图片下载(服务)";
            ...
            try
            {
                ...
                if (bingImageDownloadService.BingImageSetting != null)
                {
                    try
                    {
                        bool result = await bingImageDownloadService.WindowsSpotlightDownloadAsync();

                        if (result)
                        {

                            ClassLibrary.MyLogHelper.LogSplit(ClassLibrary.ShareClass._logPath, logTitle, $"Windows聚焦API图片下载完成。{result}", logType, logCycle);
                        }
                        else
                        {
                            ClassLibrary.MyLogHelper.LogSplit(ClassLibrary.ShareClass._logPath, logTitle, $"Windows聚焦API图片下载失败。{result}", "Error_" + logType, logCycle);
                        }
                    }
                    catch (Exception ex)
                    {
            ...
            {
                ClassLibrary.MyLogHelper.LogSplit(ClassLibrary.ShareClass._logPath, logTitle, " Windows聚焦API图片下载 服务已取消", logType, logCycle);
