		...
		internal static async Task WindowsSpotlightDownloadServiceAsync(CancellationToken cancellationToken)
		{
			...
			try
			{
				...
				if (bingImageDownloadService.BingImageSetting != null)
				{
					try
					{
						if (await bingImageDownloadService.WindowsSpotlightDownloadAsync())
						{

							ClassLibrary.MyLogHelper.LogSplit(ClassLibrary.ShareClass._logPath, logTitle, "Windows聚焦API图片下载完成。", logType, logCycle);
						}
						else
						{
							ClassLibrary.MyLogHelper.LogSplit(ClassLibrary.ShareClass._logPath, logTitle, "Windows聚焦API图片下载失败。", "Error_" + logType, logCycle);
						}
					}
					catch (Exception ex)
					{
